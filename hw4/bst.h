...
protected:
    ...
    int findHeight(Node<Key,Value>* ptr) const;
    bool balance(Node<Key,Value>* ptr) const;
...
{   
    ...
    if (curr->getLeft() == NULL && curr->getRight() != NULL) // when the single child is the right child
    {
        ...
        else if (curr->getParent()->getRight() == curr) //when the to be removed node is the right child of its parent
        {
            curr->getParent()->setRight(curr->getRight());
            curr->getRight()->setParent(curr->getParent());
        }
        else if (curr->getParent()->getLeft() == curr) //when the to be removed node is the left child of its parent
        {
    ...
    if (curr->getLeft() != NULL && curr->getRight() == NULL) // when the single child is the left child
    {
        if (curr->getParent() == NULL) // if no parent
        {
...
template<typename Key, typename Value>
bool BinarySearchTree<Key,Value>::balance(Node<Key,Value>* ptr) const
{
    if (ptr == NULL)
    {
        return true;
    ...
    int heightL = findHeight(ptr->getLeft());
    int heightR = findHeight(ptr->getRight());
    if (abs(heightL-heightR) <= 1)
    {   
        if(balance(ptr->getRight()) == true && balance(ptr->getLeft()) == true)
        {
            return true;
        }
    }
...
template <typename Key, typename Value>
int BinarySearchTree<Key,Value>::findHeight(Node<Key,Value>* ptr) const
{
    if (ptr == NULL)
    {
        return 0;
    }
    return 1 + std::max(findHeight(ptr->getLeft()), findHeight(ptr->getRight()));
}
